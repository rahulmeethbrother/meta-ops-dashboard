...
function Builder() {
  ...
  const plan = useMemo(() => {
    ...
    return { campaigns: campaignCount * copyCount, adsets: imageCount * campaignCount * copyCount, ads: imageCount * campaignCount * copyCount, daily: Number(budget || 0) * campaignCount * copyCount };
  }, [budget, copies, links]);
  ...
  return <div className="builder-page">
    ...
    <div className="builder-layout">
      <section className="builder-form card">
        ...
        <label>Image folder<input value={folder} onChange={(event) => setFolder(event.target.value)} /></label>
        ...
        <div className="builder-two"><label>Gender<select value={gender} onChange={(event) => setGender(event.target.value)}><option value="male">Men</option><option value="female">Women</option><option value="all">All genders</option></select></label><label>Budget / campaign<input type="number" min="1" value={budget} onChange={(event) => setBudget(event.target.value)} /></label></div>
        ...
        {planned && <div className="builder-success">Plan validated: {plan.campaigns} campaigns, {plan.adsets} ad sets, {plan.ads} ads, {plan.daily.toFixed(2)} daily budget. Live publishing endpoint is the next guarded step.</div>}
      </section>
      <aside className="builder-summary"><span className="ads-section-kicker">02 / PLAN</span><h2>Build footprint</h2><div className="builder-stat"><span>Campaigns</span><strong>{plan.campaigns}</strong></div><div className="builder-stat"><span>Ad sets</span><strong>{plan.adsets}</strong></div><div className="builder-stat"><span>Ads</span><strong>{plan.ads}</strong></div><div className="builder-stat"><span>Daily budget</span><strong>${plan.daily.toFixed(2)}</strong></div><p className="muted">{country} · {gender} · {minAge}–{maxAge} · {optimization}</p></aside>
    </div>
  </div>;
}
